...
public class ClientConfig implements ConfigCore {
    ...
    List<String> noPlaqueMobsRaw = KeyedValueProvider.toString(Registries.ENTITY_TYPE, EntityType.ARMOR_STAND);
    @Config(
            category = KEY_GENERAL_CATEGORY,
            name = "mob_selectors",
            description = {"Selectors for choosing mobs to render plaques for."}
    )
    ...
    public void addToBuilder(ModConfigSpec.Builder builder, ValueCallback callback) {
        this.allowRendering = builder.comment(
                "Are mob plaques enabled, toggleable in-game via the dedicated keybinding.").define("allow_rendering",
                true
        );
